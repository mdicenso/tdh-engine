...
`real_sources`: `fetch_str_market` (scarica i 10 `listings.csv.gz` + le `visualisations/reviews.csv`, ~300MB,
e proietta in **4 cache aggregate compatte** committate — `str_airbnb_territorio.csv` / `_zona.csv` /
`_roomtype.csv` / `_recensioni_mese.csv`; il grezzo per-annuncio NON è committato), `_str_italy_urls`,
`fetch_str_latest_snapshot`. La pagina ha 5 grafici: **(1)** ADR a confronto tra territori, **(2)** zone con
più annunci, **(3)** mix per tipo di alloggio, **(4)** *struttura del mercato* a confronto (% intero alloggio,
% operatori professionali = host multi-annuncio, % con licenza/CIR), **(5)** *andamento nel tempo* = recensioni
per mese come **proxy della domanda** (dal 2009: crescita + crollo COVID-2020; limite: bias di sopravvivenza).
KPI: n. annunci, ADR mediano/medio + struttura (intero/professionali/licenza/occupazione-proxy). Nota: `price`
è dell'*annuncio* (non prenotazioni reali), trattato come **EUR**; occupazione = **proxy** da `availability_365`.
Helper in `tdhlib`: `str_territori`, `str_kpi`, `str_zone`, `str_roomtypes`, `str_reviews_monthly`,
`chart_str_adr_territori`, `chart_str_zone`, `chart_str_roomtype`, `chart_str_struttura`, `chart_str_reviews`.
Registrata in Gestione dati e in matrice (riga *Affitti brevi / STR*). **Auto-refresh**:
